...
namespace TMX {
  ...
  bool Parser::load( const char* filename )
  {
	...
	if (root_node->first_attribute("backgrondcolor") != NULL &&root_node->first_attribute("backgroundcolor")->value() != 0) {
		printf(root_node->first_attribute("backgroundcolor")->value());
    ...
    for( rapidxml::xml_node<>* tileset_node = root_node->first_node( "tileset" ); tileset_node; tileset_node = tileset_node->next_sibling( "tileset" ) ) {
	  ...
	  if (tileset_node->first_attribute("source") != NULL)
	  {
		  tmpTileset.source = tileset_node->first_attribute("source")->value();
	  }
	  else
	  {
		  tmpTileset.source = "empty";
	  }
    ...
    for( rapidxml::xml_node<>* oGroup_node = root_node->first_node( "objectgroup" ); oGroup_node; oGroup_node = oGroup_node->next_sibling( "objectgroup" ) ) {
      ObjectGroup oGroup;
      std::cout << std::endl;
	  if (oGroup_node->first_attribute("color") != NULL)
	  {
	  ...
	  if (oGroup_node->first_attribute("name") != NULL)
	  {
	  ...
	  if (oGroup_node->first_attribute("opacity") != NULL)
	  {
	  ...
	  if (oGroup_node->first_attribute("visible") != NULL)
	  {
